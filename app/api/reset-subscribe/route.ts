...
export async function POST(request: Request) {
  try {
    ...
    // — Email notification to lamar@ravisphere.com (same pattern as the HIT brief) —
    try {
      await fetch("https://formsubmit.co/ajax/lamar@ravisphere.com", {
        ...
        body: JSON.stringify({
          name: name || "(no name)",
          email,
          _subject: "New 5-Day Energy Reset opt-in",
        }),
      });
    } catch (emailErr) {
      console.error("[Reset Email Notification Error]", emailErr);
    }
